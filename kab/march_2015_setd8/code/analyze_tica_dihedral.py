...
dihed = featurizer.DihedralFeaturizer(types=["phi", "psi", "chi1"])
tica = decomposition.tICA()
clusterer = cluster.MiniBatchKMeans(n_clusters=n_states)
microstate_model = msm.MarkovStateModel()
pipeline = make_pipeline(dihed, tica, clusterer, microstate_model)

pipeline.fit(trajectories0)

X = pipeline.transform(trajectories0)
Xf = np.concatenate(X)
